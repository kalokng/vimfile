...
set path+=C:/HonorFlexSDK/Version2
"set path+=C:/CVS/MGS6/FlexAIR/Code/CoreSource
"set path+=C:/CVS/MGS6/FlexAIR/Code/MainSource/Common
"set path+=C:/CVS/MGS6/FlexAIR/Code/MainSource/Main/Common
set path+=C:/cvsproject/GGAG/Source/Common/Flex
...
"                \ let project_root = substitute(fnamemodify(expand("<afile>"), ':p:h'), '\(HonorFlexSDK/Version2\)\(CVS/MGS6/FlexAIR/Code/\(CoreSource\|MainSource/\(Common\|Main\)\)\)\zs.*', '', '')|
"    autocmd bufread */PortalUI/*
"                \ let project_root = substitute(fnamemodify(expand("<afile>"), ':p:h'), 'C:/CVS/MGS6/FlexAIR/Code/MainSource/Common/\zs.*', '', '')|
"				\ echo project_root
"                \ if exists("*AddSearchPath_".&ft) | call AddSearchPath_{&ft}(project_root.'/') | endif
...
let s:function = '\(override\s\+\|abstract\s\+\|internal\s\+\|final\s\+\|private\s\+\|protected\s\+\|public\s\+\|static\s\+\)*function'
let s:class = '\(internal\s\+\|private\s\+\|protected\s\+\|public\s\+\)*class'
